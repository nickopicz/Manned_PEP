...
class CurrentMeter:
    def __init__(self, master):
        ...
        self.center_x, self.center_y = 125, 125
        self.max_value = 425
        ...
        self.canvas.configure(background='lightblue')
        self.value_label = tk.Label(master, text="0 A", font=('Helvetica', 10))
    ...
    def create_current_dial(self):
        ...
        for i in range(major_ticks + 1):
            ...
            start_x = self.center_x + (120 * math.cos(angle))
            start_y = self.center_y + (120 * math.sin(angle))
            end_x = self.center_x + (125 * math.cos(angle))
            end_y = self.center_y + (125 * math.sin(angle))
            self.canvas.create_line(
                start_x, start_y, end_x, end_y, fill="black")

            label_angle = math.radians(150 + (240 * i / major_ticks))
            label_x = self.center_x + (115 * math.cos(label_angle))
            label_y = self.center_y + (115 * math.sin(label_angle))
...
class Speedometer:
    def __init__(self, master):
        ...
        self.value_label = tk.Label(master, text="0 rpm", font=('Helvetica', 10))
...
class ThermometerGauge:
    def __init__(self, master):
        self.master = master
        self.canvas = tk.Canvas(master, width=80, height=180)
        # Adjust grid placement as needed
        self.canvas.grid(row=0, column=7, padx=20, pady=5, rowspan=2)
        self.value_label = tk.Label(
            master, text="0 °C", font=('Helvetica', 13))
        self.value_label.grid(row=1, column=7)
        self.min_temp = 0  # Minimum temperature value
        self.max_temp = 140  # Maximum temperature value
        self.gauge_height = 160
        self.gauge_width = 12
        self.gauge_x = 20
        self.gauge_y = 5
        self.draw_gauge_background()
